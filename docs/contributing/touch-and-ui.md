...
Panda now runs on touch devices (Seeed Sticky, M5Paper, M5Stack PaperMono, LilyGo T5, Xteink X4 Pro) alongside the button-only Xteink X3/X4. Every screen must work with both input styles.
...
build_flags =
  ...
  -DBOARD_HAS_PSRAM
  -DCROSSPOINT_VERSION=\"${panda.version}-m5paper\"
  -DENABLE_SERIAL_LOG
  -DLOG_LEVEL=2
...
build_flags =
  ...
  -DCROSSPOINT_VERSION=\"${panda.version}-lilygo\"
  -DENABLE_SERIAL_LOG
  -DLOG_LEVEL=2
  -DCROSSPOINT_SHOW_BUTTON_HINTS=0
lib_deps =
